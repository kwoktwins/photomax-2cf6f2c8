...

{
  ...
  negative_space_score: integer 0-25. First state in one sentence which side/area of the photo has the most open, empty space. Then score: Award 21-25 if the shot's negative space value is identical to the target's negative_space. Award 10-14 if the target is a directional value (heavy_left, heavy_right, heavy_top, heavy_bottom) and the shot is 'balanced', or vice versa. Award 0-6 if the shot's negative space is the direct opposite direction from the target on the same axis (heavy_left vs heavy_right, or heavy_top vs heavy_bottom), or if one is 'minimal' and the other is any heavy_* value.
  ...
  lighting_score: integer 0-20. First state in one sentence the specific visual cues you observe (shadow hardness, color temperature, exposure evenness, highlight/shadow balance). Then classify using these definitions: golden_hour = warm amber tones, low sun angle, long soft shadows. harsh_direct = hard-edged shadows, high contrast, visible highlight blowout. soft_diffused = even tone, minimal shadow definition. backlit_silhouette = subject appears dark against a bright background. low_key_dark = majority of frame dark/shadowed with small controlled highlights. bright_even = uniform bright exposure, no strong directional shadows. neon_artificial = visible colored light sources, unnatural color casts. Group into buckets: warm/soft = [soft_diffused, golden_hour, bright_even], harsh/dramatic = [harsh_direct, backlit_silhouette, low_key_dark, neon_artificial]. Award 17-20 if shot's lighting exactly matches target's lighting value. Award 8-12 if same bucket but different value. Award 0-5 if opposite bucket.
